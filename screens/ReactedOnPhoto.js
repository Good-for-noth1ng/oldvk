...
const ReactedOnPhoto = ({ navigation, route }) => {
  ...
  const { ownerId, photoId } = route.params

  const fetchUsersWhoReacted = async () => {
    const likesListUrl = `https://api.vk.com/method/likes.getList?access_token=${accessToken}&v=5.131&type=photo&count=${count}&offset=${offset.current}&owner_id=${ownerId}&item_id=${photoId}`
